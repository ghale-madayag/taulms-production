<template>
    ...
    <Breadcrumbs pageTitle="All Quizzes"/>
                    ...
                    <div class="card mb-0">
                        <div class="card-body">
                            <form>
                                ...
                                <div class="form-group">
                                    <label class="form-label" for="inlineFormQuiz">Quiz</label>
                                    <div class="search-form search-form--dark">
                                        <input v-model="quiz" type="text" class="form-control" placeholder="Search quiz..." id="inlineFormQuiz">
                            ...
                            <div v-for="quiz in quizzes.data" class="list-group-item d-flex flex-column flex-sm-row align-items-sm-center px-12pt">
                                <div class="media flex align-items-center mr-sm-16pt mb-8pt mb-sm-0">
                                    ...
                                    <div class="media-body">
                                        <div class="d-flex flex-column">
                                            <p class="mb-0"><strong class="js-lists-values-employee-name">{{quiz.title.substring(0,30)+"..." }}</strong></p>
                                            <small class="js-lists-values-employee-email text-50">{{ quiz.user.full_name }} | {{ quiz.lesson.title.substring(0,15)+"..." }}</small>
                                        </div>
                                    </div>
                                </div>
                                <div class="d-flex align-items-center flex-column flex-sm-row posts-card__meta">
                                    <div class="mr-3 text-50 text-uppercase posts-card__tag d-flex align-items-center">
                                        <i class="material-icons text-muted-light mr-1">school</i> {{ quiz.lesson.section.name }}
                                    </div>
...
let quiz = ref(props.filters.quiz);
...
watch([subject, faculty, section, lesson, quiz, status], debounce(function (value){
    Inertia.get(
        '/quiz/all',
        {subject: value[0],faculty: value[1], section: value[2], lesson: value[3], quiz: value[4], status: value[5]},
        {
            preserveState: true,
            replace: true
        })
